...
> **Deferred to v2** — Token payment requirements not implemented in v1 MVP.

- [ ] **PAYM-01**: Creating a poll costs 5 NIGHT tokens (tNIGHT on testnet) — fee collected atomically by the smart contract at poll creation time *(deferred)*
- [ ] **PAYM-02**: The fee amount is defined as a single named constant in `lib/midnight/fee-config.ts` — changing it requires no other code edits *(deferred)*
- [ ] **PAYM-03**: The token denomination (tNIGHT on Preview/preprod, NIGHT on mainnet) is derived from the detected network ID — no hardcoded token strings in business logic *(deferred)*
- [ ] **PAYM-04**: The Create Poll UI shows the cost (e.g. "Creating this poll costs 5 tNIGHT") before the user submits *(deferred)*
- [ ] **PAYM-05**: Insufficient token balance produces a clear, distinct error message — not a generic transaction failure *(deferred)*
...
| PAYM-01 | — | Deferred to v2 |
| PAYM-02 | — | Deferred to v2 |
| PAYM-03 | — | Deferred to v2 |
| PAYM-04 | — | Deferred to v2 |
| PAYM-05 | — | Deferred to v2 |
